...
def recognition():
    ...
    image = base64.b64decode(str(resp["image"]))       
    fileName = 'test.png'

    imagePath = ('C:\\Projects\\Tcc-4-Ano\\Recognition Api\\images\\'+'Placa'+str(idx+1)+'.png')
    png = Image.open(io.BytesIO(image))
    png.save(imagePath, 'png')

    pytesseract.pytesseract.tesseract_cmd = 'C:\\Program Files\\Tesseract-OCR\\tesseract.exe'

    img = cv2.imread("./images/"+"Placa"+str(idx+1)+".png",cv2.IMREAD_COLOR)
    ...
    # approximate the contour
        ...
        if len(approx) == 4: #chooses contours with 4 corners
            ...
            cv2.imwrite('./plates/RecognitionResult-'+str(idx+1)+'.png',new_img) #stores the new image
            break
